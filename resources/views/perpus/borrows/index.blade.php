...
@section('content')
    <div class="grid-padding text-center">
        ...
        <div class="my-2 text-right">
            ...
            <a href="{{ route('perpusku.borrow.print') }}" class="btn btn-sm btn-success">Print</a>
        </div>
                        ...
                        <td class="p-4">
                            @if(auth()->user()->role !== 'member')
                                ...
                                @elseif($borrow->status == 'booking')
                                    ...
                                    <form action="{{ route('perpusku.borrow.destroy',$borrow) }}"
                    ...
                    <form action="{{ route('perpusku.borrow.index') }}" method="POST" enctype="multipart/form-data">
                        ...
                        @if (auth()->user()->role == 'member')
                            <div class="form-group">
                                ...
                                <input type="hidden" class="form-control mb-4" placeholder="nama peminjam"
                                       name="user_id" value="{{ old('user_id', auth()->id()) }}">
                                ...
                                <select name="user_id" id="user_id" class="form-control">
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}"
                                                data-thumbnail="https://ui-avatars.com/api/background=random">
                                            {{ $user->name  }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="hidden" class="form-control mb-4" placeholder="tanggal_pinjam"
                                   name="tanggal_pinjam"
                                   value="{{ old('tanggal_pinjam', date('Y-m-d', strtotime(Carbon\Carbon::today()->toDateString())) ) }}">
                            <div
                                class="alert alert-dark rounded">{{ date('d M Y', strtotime(Carbon\Carbon::today()->toDateString())) }}</div>
                        ...
                        <div class="form-group">
                            ...
                            <div
                                class="alert alert-success rounded">{{ date('d M Y', strtotime(Carbon\Carbon::today()->addDays(7)->toDateString())) }}</div>
