...
@section('content')
    <div class="grid-padding text-center">
        ...
        @if (request('ubah'))
            <div class="container mb-4">
                <div class="card" style="position: relative;">
                    <a href="{{ route('perpusku.profil.index') }}" class="close"
                    ...
                    <div class="card-body">
                        <form action="{{ route('perpusku.profil.update', $user) }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control mb-4" placeholder="Nama" name="name"
        ...
        @else
            ...
            <ul>
                <hr>
                <li class="decoration-none text-muted" style="list-style: none">
                    <p>Email: {{ $user->email }}</p>
                </li>
                <hr>
                <li class="decoration-none text-muted" style="list-style: none">
                    <p>Role: {{ $user->role == 'petugas' ? 'admin' : $user->role }}</p>
                </li>
                <hr>
                @if ($user->role == 'member')
                    <li class="decoration-none text-muted" style="list-style: none">
                        <p>Denda: <b>Rp. </b> {{ number_format($user->denda) }}, 00</p>
                    </li>
                    <hr>
                @endif
            </ul>
                                    ...
                                    @if ($user->denda !== 0)
                                        <form action="{{ route('perpusku.profil.index') }}" method="GET">
                                            ...
                                            <button type="submit" class="btn btn-success btn-sm"
                                                    style="position: absolute;right:0px; top:20px;">Bayar Denda !
                                            </button>
                                        </form>
                                    @else
                                        <button type="button" disabled class="btn btn-secondary btn-sm"
                                                style="position: absolute;right:0px;">Tidak ada Denda !
                                        </button>
                                    @endif
                                </div>
                            @endforeach
